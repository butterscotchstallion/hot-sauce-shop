...
func (tq *TagQuery) QueryInventory() *InventoryQuery {
	query := (&InventoryClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withInventory: tq.withInventory.Clone(),
...
func (tq *TagQuery) WithInventory(opts ...func(*InventoryQuery)) *TagQuery {
	query := (&InventoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withInventory = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withInventory; query != nil {
		if err := tq.loadInventory(ctx, query, nodes,
			func(n *Tag) { n.Edges.Inventory = []*Inventory{} },
			func(n *Tag, e *Inventory) { n.Edges.Inventory = append(n.Edges.Inventory, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadInventory(ctx context.Context, query *InventoryQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Inventory)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.InventoryTable)
		s.Join(joinT).On(s.C(inventory.FieldID), joinT.C(tag.InventoryPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.InventoryPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.InventoryPrimaryKey[1]))
