...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldUsername, user.FieldPassword, user.FieldAvatarFilename:
...
func (u *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldAvatarFilename:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field avatarFilename", values[i])
			} else if value.Valid {
				u.AvatarFilename = value.String
			}
