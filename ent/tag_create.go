...
func (tc *TagCreate) AddInventoryIDs(ids ...int) *TagCreate {
	tc.mutation.AddInventoryIDs(ids...)
	return tc
}

// AddInventory adds the "inventory" edges to the Inventory entity.
func (tc *TagCreate) AddInventory(i ...*Inventory) *TagCreate {
	ids := make([]int, len(i))
	for j := range i {
		ids[j] = i[j].ID
	}
	return tc.AddInventoryIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.InventoryIDs(); len(nodes) > 0 {
